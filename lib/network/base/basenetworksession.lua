...
function BaseNetworkSession:init()
	...
	do
		local (for generator), (for state), (for control) = ipairs(self.MATCHMAKING_SERVER_IP_ADDRESSES)
		do
			do break end
...
function BaseNetworkSession:_update_matchmaking_server_connection(wall_run_t)
	if wall_run_t > self._next_NAT_keep_alive_t then
		...
		elseif self._my_external_IP then
			local (for generator), (for state), (for control) = pairs(self._matchmaking_server_rpcs)
			do
				do break end
				rpc:NAT_keep_alive()
			end
