...
function TimelineManager:event_ids()
	local t = {}
	do
		local (for generator), (for state), (for control) = ipairs(Global.timeline_manager.event_list)
		do
			do break end
		...
		do
			...
			if child_node_name == "checkpoints" then
				if data.checkpoint_list then
					...
					local (for generator), (for state), (for control) = ipairs(child_node)
					do
						do break end
...
function TimelineManager:_load_debug_dynamic_data(event_index)
	...
	for index = 1, event_index do
		...
		if debug_dynamic_data then
			if debug_dynamic_data.add_inventory_list then
				local (for generator), (for state), (for control) = ipairs(debug_dynamic_data.add_inventory_list)
				do
					do break end
			...
			if debug_dynamic_data.remove_inventory_list then
				local (for generator), (for state), (for control) = ipairs(debug_dynamic_data.remove_inventory_list)
				do
					do break end
	...
	if Global.category_print.timeline_manager then
		...
		do
			local (for generator), (for state), (for control) = pairs(inventory_map)
			do
				do break end
