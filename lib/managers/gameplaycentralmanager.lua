...
function GamePlayCentralManager:update(t, dt)
	if #self._contour.units > 0 then
		...
		if data.color ~= data.target_color then
			...
			local (for generator), (for state), (for control) = ipairs(data.materials)
			do
				do break end
	...
	do
		local (for generator), (for state), (for control) = pairs(self._enemy_contour_units)
		do
			do break end
			local unit = data.unit
...
function GamePlayCentralManager:remove_contour_unit(unit)
	do
		local (for generator), (for state), (for control) = pairs(self._contour.units)
		do
			do break end
	...
	if self._camera_pos then
		...
		if dis < 4000 then
			if #self._footsteps > 2 then
				...
				do
					local (for generator), (for state), (for control) = ipairs(self._footsteps)
					do
						do break end
