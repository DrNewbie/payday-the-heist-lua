	...
	do
		...
		do
			...
			if event.checkpoint_list then
				local (for generator), (for state), (for control) = ipairs(event.checkpoint_list)
				do
					do break end
...
function MenuPlayerInventory:modify_player_inventory(node)
	...
	do
		local (for generator), (for state), (for control) = ipairs(slots)
		do
			do break end
	...
	do
		...
		do
			...
			if slot:item() then
				...
				item_name = "<empty>"
				local (for generator), (for state), (for control) = ipairs(slot:categories())
				do
					do break end
...
function MenuPlayerInventory:equippable_slot_items(node, inventory_slot)
	...
	do
		local (for generator), (for state), (for control) = ipairs(inventory_slot:categories())
		do
			do break end
	...
	do
		local (for generator), (for state), (for control) = ipairs(tmp_equippable_items)
		do
			do break end
			local (for generator), (for state), (for control) = ipairs(cat_table)
			do
				do break end
	...
	do
		local (for generator), (for state), (for control) = ipairs(equippable_items)
		do
			do break end
	...
	if item then
		...
		local menu_item = new_node:create_item(nil, params)
		new_node:add_item(menu_item)
		local (for generator), (for state), (for control) = pairs(managers.item:item_upgrade_map())
		do
			do break end
...
function MenuPlayerInventory:slot_item_upgrades(node, item)
	...
	if item_upgrade_list then
		local (for generator), (for state), (for control) = ipairs(item_upgrade_list)
		do
			do break end
...
function MenuLoadGameCreator:modify_node(original_node)
	...
	do
		local (for generator), (for state), (for control) = pairs(save_info_list)
		do
			do break end
...
function MenuDialogOptions:modify_node(node, ...)
	...
	do
		local (for generator), (for state), (for control) = pairs(option_list)
		do
			do break end
			local params = {
				name = item.string_id,
