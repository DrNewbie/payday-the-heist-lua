...
function PlayerManager:set_crew_bonus(peer_id, upgrade, level)
	...
	do
		local (for generator), (for state), (for control) = ipairs(self._global.synced_bonuses)
		do
			do break end
...
function PlayerManager:_internal_load()
	...
	do
		local (for generator), (for state), (for control) = ipairs(self._global.kit.weapon_slots)
		do
			do break end
	...
	do
		local (for generator), (for state), (for control) = ipairs(self._global.default_kit.crew_bonus_slots)
		do
			break
		end
...
function PlayerManager:availible_crew_bonuses(slot)
	...
	local bonuses = {}
	do
		local (for generator), (for state), (for control) = pairs(self._global.upgrades.crew_bonus)
		do
			do break end
...
function PlayerManager:_flush_item_queue(player_index, player)
	if self._queued_item_map then
		...
		if queued_player_item_map then
			do
				local (for generator), (for state), (for control) = pairs(queued_player_item_map)
				do
					do break end
					player:outfit():equip(item, slot_name)
				end
