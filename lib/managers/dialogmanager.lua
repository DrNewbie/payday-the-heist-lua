...
function DialogManager:conversation_names()
	local t = {}
	do
		local (for generator), (for state), (for control) = pairs(self._conversation_list)
		do
			do break end
			table.insert(t, name)
		end
...
function DialogAction:setup_variable(data_node)
	...
	if data_node.variable then
		...
		do
			local (for generator), (for state), (for control) = pairs(condition_list)
			do
				do break end
				if data_node[k] then
...
function DialogNodeSelect:update(...)
	local option_list = {}
	do
		local (for generator), (for state), (for control) = pairs(self._action_list)
		do
			do break end
			local data = k:data()
			if data then
