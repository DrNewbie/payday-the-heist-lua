...
function UpgradesManager:setup_current_weapon()
	...
	local (for generator), (for state), (for control) = ipairs(aquired_updgrades)
	do
		do break end
...
function UpgradesManager:_apply_visual_weapon_upgrade(weapon_unit, upgrade)
	...
	do
		local (for generator), (for state), (for control) = pairs(upgrade.objs)
		do
			do break end
...
function UpgradesManager:alternative_upgrades()
	local t = {}
	do
		local (for generator), (for state), (for control) = ipairs(self._global.progress)
		do
			do break end
