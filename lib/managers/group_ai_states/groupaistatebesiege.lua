...
function GroupAIStateBesiege:_begin_new_tasks()
	...
	do
		...
		do
			...
			if area_data.spawn_points and not all_nav_segs[nav_seg].disabled then
				local (for generator), (for state), (for control) = pairs(area_data.spawn_points)
				do
					do break end
	...
	repeat
		...
		if nr_criminals == 0 then
			local (for generator), (for state), (for control) = pairs(all_nav_segs[search_seg].neighbours)
			do
				do break end
	...
	if recon_candidates and #recon_candidates > 0 then
		...
		do
			local (for generator), (for state), (for control) = ipairs(recon_candidates)
			do
				do break end
				do
					local (for generator), (for state), (for control) = ipairs(managers.enemy:all_civilians())
					do
						do break end
...
function GroupAIStateBesiege:_find_surplus_cops_around_area(nav_seg, wanted_nr_units, spawn_threshold)
	...
	repeat
		...
		do
			local (for generator), (for state), (for control) = pairs(police_units)
			do
				do break end
...
function GroupAIStateBesiege:_remove_preferred_spawn_point_from_area(area, sp_data)
	local area_data = self._area_data[area]
	local (for generator), (for state), (for control) = ipairs(area_data)
	do
		do break end
...
function GroupAIStateBesiege:_create_area_data()
	...
	local (for generator), (for state), (for control) = pairs(self._spawn_points)
	do
		do break end
		self:_map_spawn_points_to_respective_areas(id, spawn_points)
	end
	...
	do
		...
		do
			...
			if nr_units > 0 then
				...
				local (for generator), (for state), (for control) = ipairs(u_positions)
				do
					do break end
					local brush
