...
function GroupAIStateBase:set_AI_enabled(state)
	...
	if not state then
		...
		local (for generator), (for state), (for control) = ipairs(all_weapons)
		do
			do break end
...
function GroupAIStateBase:propagate_alert(alert_data)
	...
	do
		local (for generator), (for state), (for control) = ipairs(entries_com)
		do
			do break end
	...
	local (for generator), (for state), (for control) = pairs(self._alert_elements)
	do
		do break end
...
function GroupAIStateBase:remove_special_objective(id)
	...
	if #area_so == 1 then
		...
		local (for generator), (for state), (for control) = ipairs(area_so)
		do
			do break end
			if so_id == id then
