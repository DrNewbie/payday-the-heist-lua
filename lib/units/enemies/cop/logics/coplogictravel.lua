...
function CopLogicTravel.reset_actions(data, internal_data, old_internal_data, allowed_actions)
	...
	for body_part = 1, 4 do
		...
		if active_action then
			...
			local (for generator), (for state), (for control) = ipairs(allowed_actions[body_part])
			do
				do break end
	...
	do
		local (for generator), (for state), (for control) = ipairs(active_actions)
		do
			do break end
...
function CopLogicTravel.chk_should_turn(data, my_data)
	if not my_data.advancing and not my_data.turning then
