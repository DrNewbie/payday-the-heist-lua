	...
	do
		...
		do
			...
			if not record then
				...
				if not enemy_data.is_deployable then
					-- unhandled boolean indicator
				else
					local reviving = true
				end
...
function CopLogicAttack.chk_should_turn(data, my_data)
	if not my_data.turning and not data.unit:movement():chk_action_forbidden("walk") and not my_data.moving_to_cover and not my_data.walking_to_cover_shoot_pos then
		-- unhandled boolean indicator
	else
	end
